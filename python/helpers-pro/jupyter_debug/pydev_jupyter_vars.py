...
def table_command(command_text):
    ipython_shell = get_ipython()
    namespace = ipython_shell.user_ns
    command, command_type, start_index, end_index = command_text.split(NEXT_VALUE_SEPARATOR)
    ...
    status, res = exec_table_command(command, command_type, start_index, end_index, namespace, namespace)
    print(res)
